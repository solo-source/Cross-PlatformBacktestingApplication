...
class WsBacktestWindow(BaseBacktestWindow):
    ...
    def _build_results_ui(self):
        """
        Backtest Results tab: a Matplotlib canvas + navigation toolbar.
        """
        self.equity_canvas = MplCanvas(self, width=5, height=4, dpi=100)
        self.results_layout.addWidget(self.equity_canvas)

        nav_toolbar = NavBar(self.equity_canvas, self)
        self.results_layout.addWidget(nav_toolbar)
    ...
    #     If “Date” is not a Timestamp, coerce it via pd.to_datetime.
    #     """
    ...
    #         self.table.setItem(r, 1, QTableWidgetItem(f"{row['Open']:.2f}"))
    #         self.table.setItem(r, 2, QTableWidgetItem(f"{row['High']:.2f}"))
    #         self.table.setItem(r, 3, QTableWidgetItem(f"{row['Low']:.2f}"))
    #         self.table.setItem(r, 4, QTableWidgetItem(f"{row['Close']:.2f}"))
    #         self.table.setItem(r, 5, QTableWidgetItem(str(int(row["Volume"]))))
